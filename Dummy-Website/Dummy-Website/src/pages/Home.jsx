...
//   console.log(filteredLaunches);
  ...
  useEffect(() => {
      ...
      if (statusFilter === "successful") {
          filtered = filtered.filter(launch => launch.launch_success === true && launch.upcoming === false);
      } else if (statusFilter === "failed") {
          filtered = filtered.filter(launch => launch.launch_success === false && launch.upcoming === false);
      ...
      setCurrentPage(1);
  }, [dateFilter, statusFilter, launches]);

  const totalPages = Math.ceil(filteredLaunches.length / itemsPerPage);
  const currentLaunches = filteredLaunches.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
      ...
      <h1 className="text-xl font-bold ">SpaceX Explorer</h1>
         <div className="p-6 bg-gray-900 w-[100%] text-sky-400">
            ...
            <div className="mt-4 flex justify-center space-x-2">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="px-4 py-2 bg-gray-700 text-white disabled:opacity-50">Prev</button>
                <span>Page {currentPage} of {totalPages}</span>
                <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="px-4 py-2 bg-gray-700 text-white disabled:opacity-50">Next</button>
            </div>
        </div>
